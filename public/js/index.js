...
var PowerSwitch = 1;//0:关，1:开
var weat = ['sun','thunder','snow','wind'];
var weatData = 0;

//初始化的时候窗户打开
$.get("/tuy.set", { value:PowerSwitch },function(data){
	if(data.code === 200){
	console.log('返回数据：',data.data.value);	
	$('#openStatus').html('开启');
	$('#PowerSwitch').html(1);
}});

$('#open').click(function(){
	$.get("/tuy.set", { value:1 },function(data){
		if(data.code === 200){
			$('#openStatus').html('开启');
			$('#PowerSwitch').html(1);		
	}});
...
$('#close').click(function(){
	$.get("/tuy.set", { value:0 },function(data){
		if(data.code === 200){
			$('#openStatus').html('关闭');
			$('#PowerSwitch').html(0);
	}});
...
setInterval(()=>{
	...
	//每秒钟来请求服务器，不虚
	$.get("/tuy.get", { name: "SmartCarWindow" },function(data){
		if(data.code === 200){
			if(weatData !== data.data.value){
				weatData = data.data.value;
				$('#button-' + weat[data.data.value]).trigger('click');
			    $('#WaterSensorState').html(data.data.value);
			}
			console.log('返回数据：',data.data.value)
...
var weather = [
	...
	//{ type: 'rain', name: '小雨'}, 
	{ type: 'sun', name: '天晴',temp:'30'},
	{ type: 'thunder', name: '下雨',temp:'20'},
	{ type: 'snow', name: '下雪',temp:'-12'}, 
	{ type: 'wind', name: '台风',temp:'10'},
];
...
function updateSummaryText()
{
	...
	temperature.html(currentWeather.temp);
